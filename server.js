...
const WRITE_ENABLED = /^(1|true|yes)$/i.test(process.env.CODEX_ARCHIVE_MANAGER_WRITE || '');
...
function requireWriteEnabled() {
  ...
  const err = new Error('Write mode is disabled. Restart with CODEX_ARCHIVE_MANAGER_WRITE=1 or npm run start:write to restore, delete, or remove records.');
  err.statusCode = 403;
  throw err;
}
    ...
    <div class="shell">
      <div class="topbar">
        <div class="brand">
          ...
          <div class="safety-notice" id="safetyNotice">Read-only mode. This tool can inspect local Codex records; start with npm run start:write to enable changes.</div>
        </div>
      </div>
    </div>
  </header>
  ...
  <script>
    ...
    function setWriteMode(enabled) {
      ...
      safetyNotice.textContent = writeEnabled
        ...
        : 'Read-only mode. This tool can inspect local Codex records; start with npm run start:write to enable changes.';
    }
      ...
      rows.innerHTML = items.map(item => {
        ...
        const restoreButton = item.status === 'archived' && item.exists
          ? writeEnabled
            ...
            : '<button class="ghost" disabled title="Start with npm run start:write to enable restore">Restore</button>'
          : '';
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    json(res, err.statusCode || 500, { error: err.message || String(err) });
  }
});
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(WRITE_ENABLED ? 'Write mode enabled' : 'Read-only mode');
});
